...
fillRect(x,y,width,height,bgColor){if(bgColor!==undefined){this.setFillStyle(bgColor);}
this.contextB.fillRect(x,y,width,height);}
setFillStyle(col){var style=getString(col);if(style!=this.currentState.fillStyle){this.contextB.fillStyle=style;this.currentState.fillStyle=style;}}
startBuffering(){this.contextB.beginPath();}
moveTo(x,y){this.contextB.moveTo(x,y);}
prepareRect(x,y,width,height){this.contextB.rect(x,y,width,height);}
finishRects(){this.contextB.fill()}
...
playCallback(lookAhead){if(this.playing){var now=this.currentTime;var realNow=this.realTime;var nextTime=Math.floor(now+lookAhead*this.currentMidi.header.divisions.true_value);var ind=this.songPointer;var notes=this.activeNotes;while(now<=nextTime){if(ind<this.usableEvents.length){var e=this.usableEvents[ind];var id;var eType=e.type;realNow+=(e.delta-now)/ this.currentMidi.header.divisions.true_value /(this.info.tempoBPM / 60);now=e.delta;this.processEvent(e,realNow);ind++;}else{this.playing=false;ind=0;now=0;break;}}
...
function newDrawNotes(time){var sorts=midiController.activeNotes.slice().sort(function(a,b){if(a===null){return 1;}else if(b===null){return-1;}else if(colorMode){return a.track-b.track;}
return a.channel-b.channel;});var groups=[];var room=gfxController.height-key0Height;for(var i=0;i<sorts.length;i++){if(sorts[i]){var startTime=sorts[i].startTime;var endTime=sorts[i].endTime;var xPos=getKey(sorts[i].note).x;var width=getKey(sorts[i].note).width;var yPos;var height;if(endTime>-1){yPos=Math.floor(room*(1-(endTime-time)/ 2));}else{yPos=0;}
if(startTime-time>2){height=0;}else{var yy=room*(1-(startTime-time)/ 2);var height=Math.floor(yy-yPos);}
if(i==0){groups.push(new Array());if(colorMode){groups[groups.length-1].push({col:sorts[i].track,x:xPos,y:yPos,w:width,h:height});}else{groups[groups.length-1].push({col:sorts[i].channel,x:xPos,y:yPos,w:width,h:height});}}else{if(colorMode){if(sorts[i].track!=sorts[i-1].track){groups.push(new Array());}
groups[groups.length-1].push({col:sorts[i].track,x:xPos,y:yPos,w:width,h:height});}else{if(sorts[i].channel!=sorts[i-1].channel){groups.push(new Array());}
groups[groups.length-1].push({col:sorts[i].channel,x:xPos,y:yPos,w:width,h:height});}}}}
for(var g=0;g<groups.length;g++){var indG=groups[g];var indC=groups[g][0].col;gfxController.setFillStyle(usableColors[indC][1]);gfxController.startBuffering();gfxController.moveTo(0,0);for(var i=0;i<indG.length;i++){gfxController.prepareRect(indG[i].x,indG[i].y,indG[i].w,indG[i].h);}
gfxController.finishRects();gfxController.setFillStyle(usableColors[indC][0]);gfxController.startBuffering();gfxController.moveTo(0,0);for(var i=0;i<indG.length;i++){gfxController.prepareRect(indG[i].x+3,indG[i].y+3,indG[i].w-6,indG[i].h-6);}
gfxController.finishRects();}}
function update(time){var notes=midiController.activeNotes;var pressed=new Array();for(var i=0;i<notes.length;i++){var note=notes[i];if(note){if(note.startTime<time&&(note.endTime==-1||note.endTime>time)){pressed.push(note.note);if(colorMode){getKey(note.note).color=usableColors[note.track][0];}else{getKey(note.note).color=usableColors[note.channel][0];}
...
prevPressed=pressed;}
function draw(){gfxController.clearRect(0,0,gfxController.width,gfxController.height-key0Height-1);var t=audioController.getTime();update(t);newDrawNotes(t);for(var i=0;i<keys.length;i++){var tmpkey=keys[i];gfxController.fillRect(tmpkey.x,tmpkey.y,tmpkey.width,tmpkey.height,tmpkey.color);gfxController.drawRect(tmpkey.x,tmpkey.y,tmpkey.width,tmpkey.height,tRGB(0,0,0));}
gfxController.swapBuffers();requestAnimationFrame(draw);if(midiController.playing){if(midiController.realTime-t<4||midiController.realTime<t){midiController.playCallback(Math.abs(midiController.realTime-t));}}}
